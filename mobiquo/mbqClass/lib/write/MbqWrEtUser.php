...
Class MbqWrEtUser extends MbqBaseWrEtUser {
    ...
    public function registerUser($username, $password, $email, $verified, $custom_register_fields, $profile, &$errors) {
        ...
        if (!sizeof($errors))
        {
            ...
            $auto_approve = 0; // do not auto approve in ttg, however we still need auto approve check from public plugin
            if(($auto_approve && $verified && $config['require_activation'] != USER_ACTIVATION_ADMIN) || ($config['require_activation'] == USER_ACTIVATION_NONE )
            ...
            else
            {
				...
				if ($config['email_enable'] && $email_template)
				{
                    ...
                    $messenger->assign_vars([
                        'USER_AVATAR' => $user::getAvatarUrl($user_id),
                        'USER_NAME' => htmlspecialchars_decode($data['username']),
                        'USER_EMAIL' => htmlspecialchars_decode($data['user_email']),
                    ]);
                ...
                {
                    $phpbb_notifications = $phpbb_container->get('notification_manager');
					$phpbb_notifications->add_notifications('notification.type.admin_new_user', array(
						'user_id'		=> $user_id,
                        'user_ip'       => $user->ip,
						'user_regdate'	=> $user_row['user_regdate'],
					));
